...
class Categorizer(object):
    """
        Calculate if the variables are over or under represented with respect to the target
        Categorize the variables into category according to the target
        for binary target only

    """
    def __init__(self, df, target_col, num_bins=5):
    ...
    def get_likelihood(self, colname, categorical=False):
        "get the likelihood reponse rate for the given column"
    ...
    def categorize(self, sdf=None, exclusions=None):
